...
public class QuizActivity extends AppCompatActivity implements View.OnClickListener, ExoPlayer.EventListener {
    ...
    // cd (3): Add conditional logging statements to the onPlayerStateChanged() method that log when ExoPlayer is playing or paused.
    @Override
    public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
        if(playbackState == STATE_READY && playWhenReady ){
            Log.d(TAG, "We're playing!");
        } else if(playbackState == STATE_READY ){
            Log.d(TAG, "We're paused!");
